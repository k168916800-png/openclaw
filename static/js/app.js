...
createApp({
    ...
    methods: {
        ...
        parseVlessLink() {
            if (!this.vlessLink.trim()) {
                this.showToast("请粘贴 VLESS 链接", "warning");
                return;
            }

            try {
                const link = this.vlessLink.trim();
                const vlessMatch = link.match(/^vless:\/\/([^@]+)@([^:]+):(\d+)(\?[^#]*)?(#.*)?$/);
                
                if (!vlessMatch) {
                    this.showToast("❌ VLESS 链接格式不合法，请检查", "error");
                    this.vlessParsed = null;
                    return;
                }

                const user = vlessMatch[1] || '';
                const host = vlessMatch[2];
                const port = vlessMatch[3];
                const queryStr = vlessMatch[4] || '';
                const remarks = vlessMatch[5] ? vlessMatch[5].substring(1) : '';

                // 解析查询参数
                const params = new URLSearchParams(queryStr.substring(1));
                const security = params.get('security') || 'none';
                const sni = params.get('sni') || '';
                const tlsFingerprint = params.get('fp') || '';
                ...
                this.showToast("✅ VLESS 链接解析成功，请点击【应用此代理】", "success");
            } catch (e) {
                console.error("解析 VLESS 链接出错:", e);
        ...
        applyVlessProxy() {
            if (!this.vlessParsed) {
                this.showToast("请先解析 VLESS 链接", "warning");
                return;
            }

            this.config.default_proxy = this.vlessParsed.proxyUrl;
            this.showToast(`✅ 已应用代理: ${this.vlessParsed.proxyUrl}`, "success");
